            ...
            try
            {
                ...
                for (int r = 2; r <= data.GetLength(0); r++)
                {
                    for (int c = 1; c <= data.GetLength(1); c++)
                    {
                        ...
                        if (c == 1)
                        {
                            str1 = (string)(rng.Cells[r, c] as Range).Value2; // 열과 행에 해당하는 데이터를 문자열로 반환
                        }
                        else if (c == 2)
                        {
                            num1 = (int)(rng.Cells[r, c] as Range).Value2; // 열과 행에 해당하는 데이터를 문자열로 반환
                            string str2 = string.Format("{0}", num1);
                        }
                        else
                        {
                            str3 = (string)(rng.Cells[r, c] as Range).Value2; // 열과 행에 해당하는 데이터를 문자열로 반환
                        }
                    }
                    BuyDate buy = new BuyDate(str1, num1, str3);
                    buylist.Add(buy);
                    // Data 빼오기
